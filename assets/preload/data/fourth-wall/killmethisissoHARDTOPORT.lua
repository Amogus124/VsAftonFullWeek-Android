function onStepHit()
    if curStep == 544 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
     if curStep == 547 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 552 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 555 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 560 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 563 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 568 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 704 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 707 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 712 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 715 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 720 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 723 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 728 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 732 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 768 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 771 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
        if curStep == 776 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
        if curStep == 779 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 784 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 787 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 792 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 796 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 832 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 835 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 840 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 843 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
        if curStep == 848 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 851 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 856 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 860 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 928 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 931 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 936 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 939 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 944 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 947 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 952 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 956 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1216 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1219 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1224 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1227 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
        if curStep == 1232 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1235 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 1240 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1244 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1280 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1283 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1288 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1291 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1296 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
        if curStep == 1299 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
        if curStep == 1304 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
        if curStep == 1308 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
        if curStep == 1344 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1347 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1352 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1355 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1360 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1363 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1368 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1372 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1440 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1443 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 1448 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
      if curStep == 1451 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1456 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1459 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1464 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66)
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
       if curStep == 1468 then
       doTweenAlpha('dient', 'gradient1', 0, 0.55, 'linear')
       setProperty('gradient1.alpha', 0.66) --why???
       doTweenAlpha('dient1', 'gradient', 0, 0.55, 'linear')
        setProperty('gradient.alpha', 0.55)
    end
end


--FUUUUUUUUUUUUUUUU AHHHHHHHHHHHHHHHHHHHHHH